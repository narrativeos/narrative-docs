...
V1 以 [v1-prototype-spec.md](v1-prototype-spec.md) 为准：Atlas 作为中栏主舞台能力存在，与域导航同栏，不默认进入全屏。

V1 交互约束：左栏 Source Foundation 跨域稳定，域切换时不改变左栏结构与可见性；左栏源工作台采用“多选源列表 + 焦点源”模型，为后续多源对比与版本时序对比预留；Atlas 专属控制（Layer/Mode/Drill）仅在 Atlas 域显示。
