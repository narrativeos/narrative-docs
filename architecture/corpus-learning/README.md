...
若 Corpus 能力被用于研究结论而非仅用于产品观察，则至少还应满足以下约束：

- 语料集必须具有明确 `corpus_id`、版本号与样本边界
- 趋势、聚类与对比结果必须能回链到样本集合与过滤条件
- 涉及人工标签的任务必须说明标注协议或规则来源
- 正式研究输出必须附带最小复现包，而不是只给图表结论

研究型约束的统一定义见： [../../whitepaper/research-methodology-and-reproducibility.md](../../whitepaper/research-methodology-and-reproducibility.md)
