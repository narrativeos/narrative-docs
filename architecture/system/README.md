...
为避免“局部优化有效但系统不可控”，本项目统一以下可靠性约束。

| Domain | SLI | SLO (30天滚动) | Failure Definition |
| --- | --- | --- | --- |
| 交互路径（Fast Queue） | p95 首次反馈时延 | <= 2s | 连续 10 分钟超标即记为 SLO breach |
| 深度路径（Deep Queue） | 任务完成率 | >= 99.0% | 非用户取消且无结果落盘视为失败 |
| 存储路径 | manifest 一致性成功率 | >= 99.9% | manifest 与 artifact 版本不一致 |
| 跨运行时通信 | IPC 合约成功率 | >= 99.95% | 合约字段缺失/反序列化失败 |

恢复目标：

- RTO: P1 事件 30 分钟内恢复服务可用，P2 事件 4 小时内恢复。
- RPO: 默认 5 分钟（以 snapshot/manifest 发布点计算）。

Error Budget 规则：

- Fast Queue 可用性预算按 99.5% 计算，月度耗尽后冻结新特性发布，仅允许稳定性修复。
- Deep Queue 预算按 99.0% 计算，连续两周超预算必须触发容量评审与降级策略重算。
...
| --- | --- | --- | --- |
| 未授权 artifact 读取 | Host <-> Worker <-> Storage | artifact ACL、短时效 token、最小权限 | 访问审计日志 + 拒绝率指标 |
| 跨租户数据串读（若外部平台启用多租户） | 外部身份域 <-> 数据域 | tenant_id 强制传递、查询级隔离断言 | 隔离测试报告 |
| 密钥泄露/误用 | 配置与密钥管理域 | KMS 托管、按环境轮换、禁明文落盘 | 密钥轮换记录 |
| 重放与篡改 | IPC/API Contract | 请求签名、幂等键、版本校验 | 合约校验与重放拒绝日志 |

数据分级基线：

- L1 Public：公开样例与文档。
- L2 Internal：项目内分析结果与指标。
- L3 Restricted：含敏感文本或受限语料，仅在最小权限与审计开启下访问。

最小审计事件集：artifact_read_denied、manifest_publish、queue_degrade、auth_context_attached、tenant_isolation_check_failed。
